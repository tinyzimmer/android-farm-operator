...
func calculateServiceDefinitions(cr *androidv1alpha1.AndroidFarm) map[string]svcDef {
	...
	// Provider Routes
	for _, svc := range getProviderSvcDefinitions(cr) {
		for svcName, svcAttrs := range svc {
			svcs[svcName] = svcAttrs
		}
	}
	return svcs
}

func getProviderSvcDefinitions(cr *androidv1alpha1.AndroidFarm) []map[string]svcDef {
	...
	for _, group := range cr.DeviceGroups() {
		...
		for i := minPort; i <= maxPort; i++ {
			...
			svcs = append(svcs, map[string]svcDef{svcName: svcDef{
				...
				Endpoints:  []string{fmt.Sprintf("%s-%s", cr.STFNamePrefix(), providerName)},
				Priority:   "10",
